...
if [[ "${YES}" == "1" ]]; then
  ...
  echo "This will destroy the AWS hosted demo slice in ${AWS_REGION_VALUE}."
fi

terraform -chdir="${INFRA_DIR}" init
if [[ "${#EXTRA_ARGS[@]}" -gt 0 ]]; then
  terraform -chdir="${INFRA_DIR}" destroy "${TF_ARGS[@]}" "${EXTRA_ARGS[@]}"
else
  terraform -chdir="${INFRA_DIR}" destroy "${TF_ARGS[@]}"
fi

rm -f "${INFRA_DIR}/.aws-demo-outputs.json"
echo "AWS hosted slice destroyed."
